...
def train(
    ...
    history = {k: [] for k in ["train_loss", "val_loss", "train_acc", "val_acc", "val_ap", "val_p_at_100"]}
...
def plot_training_curves(history_p1: dict, history_p2: dict = None) -> None:
    ...
    for ax, (y1_key, y2_key, title, ylabel) in zip(
        axes,
        [
            ("train_loss", "val_loss",     "Loss curves",                   "Loss"),
            ("train_acc",  "val_acc",      "Accuracy curves",               "Accuracy"),
            ("val_ap",     "val_p_at_100", "Ranking-oriented val metrics",  "Score"),
        ],
    ):
        ax.plot(epochs, history[y1_key], label=y1_key.replace("_", " "), linewidth=2)
        ax.plot(epochs, history[y2_key], label=y2_key.replace("_", " "), linewidth=2)
        if phase1_end:
...
def run_full_pipeline() -> None:
    ...
    print("\nPipeline complete!")
    print("Outputs:")
    print("  training_curves.png    — loss / accuracy / AP curves")
